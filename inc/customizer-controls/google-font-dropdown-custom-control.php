 ...
 */
 class Google_Font_Dropdown_Custom_Control extends WP_Customize_Control
 {
    ...
    public function render_content()
    {
        if(!empty($this->fonts))
        {
            $font_property = $this->input_attrs['font_property'];
                ...
                <label>
                    ...
                    <select <?php $this->link(); ?>>
                        <?php
                            ...
                            {
                                printf('<option value="%s" %s>%s</option>', $v->family, selected($this->value(), $k, false), $v->family); echo selected($this->value(), $k, false);
                            }
                        ?>
                    </select>
                </label>

            <?php } else if ($font_property == 'variants') {
                $selected_font_family = get_theme_mod( 'elexis_heading_font_family' );
                $selected_font_family = function() {
                ...
                <label>
                    ...
                    <select <?php $this->link(); ?>>
                        <?php
                            foreach ( $this->fonts as $k => $v ) {
                              if ($v->family == $selected_font_family) {
                                foreach ($v->variants as $variant) {
                                  printf('<option value="%s" %s>%s</option>', $variant, selected($this->value(), $k, false), $variant);
                                }
                              }
